...
- Модель не ответила за `LLM_TIMEOUT`, ответ обрезан по `LLM_MAX_TOKENS` или не JSON — **503** с причиной. Повторов нет (кроме одного: обрыв ответа со схемой — зацикливание на пробелах между элементами JSON), на другую модель сервис не переключается.
...
| `LLM_SCHEMA_MAX_TOKENS` | `1400` | потолок ответа со схемой (classify): заполненный до всех maxLength — 1 047 токенов; обрыв — один повтор, второй — отказ |
