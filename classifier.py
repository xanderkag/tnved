...
# Рассуждение — до кода (Ж): когда код шёл первым, модель спорила с собой в reasoning, меняла
# решение и упиралась в 2048 токенов (холдинг после Д4 — 13 из 176 строк). Проба на этих строках,
# 3 прогона: обрезано 4 → 1 из 42, медиана ответа 889 → 560 токенов, верно на 10 знаках 0 → 10.
...
Сначала рассуждение, потом код: в primary.reasoning сравни 2–3 ближайших кандидата по уровням
их иерархии и реши — 2–4 предложения; код — тот, к которому пришло рассуждение. Решение не пересматривай
и рассуждений вне reasoning не пиши.

Отвечай СТРОГО в формате JSON:
{
  "primary": {
    "reasoning": "2–4 предложения: почему этот код, а не ближайшие кандидаты",
    "code": "XXXXXXXXXX",
  ...
  "alternatives": [
    {
      "code": "XXXXXXXXXX",
      "why_close": "одно предложение",
      "why_rejected": "одно предложение"
    }
  ],
  "gri_applied": ["1", "3a"],
  "checks_required": ["..."]
