...
- **Уверенность `high`** — только если модель не называет альтернативу из другой товарной позиции (4 знака); иначе `medium` и проверка в «Стоит проверить». На холдинге у такой `high` позиция верна в 53 из 54; 10 знаков — нет: подвид по короткому описанию часто не различить.
