...
- **Три UI-режима:**
  ...
  - **Пакет (Excel)** — загрузка xlsx, прогресс-бар, скачивание xlsx с результатами. В результате — колонка «Проверить»: что сверить вручную. До 500 строк, параллелизм 5. Колонки узнаются по заголовкам (шапка — в любой из первых 10 строк): описание обязательно, артикул, производитель и страна происхождения идут модели вместе с ним; колонки с кодом ТН ВЭД и пошлиной не читаются. В результате — номер строки исходного файла; строка без описания попадает туда с ошибкой, а не пропадает. Описание из одного обозначения (марка, артикул — ни одного слова) в модель не уходит: кода нет, в «Ошибка» — «код не выдан: описание — только обозначение…»; по такому модель угадывала и не попадала даже в позицию.
  - **Чат** — описание свободным текстом; если не хватает деталей — один раунд вопросов, ответ на них — и код (группа — заново по описанию с ответом). На чат не больше двух triage и одного classify.
...
- **Уверенность `high`** — только если модель не называет альтернативу из другой товарной позиции (4 знака); иначе `medium` и проверка в «Стоит проверить». Соперник из той же позиции, но другой подпозиции — уверенность не меняется, в проверки идут пути обоих кодов по тарифу без общей части: признак, которым подпозиции различаются (9401 39 — вращающаяся с регулировкой высоты, 9401 71 — металлический каркас). На холдинге у такой `high` позиция верна в 53 из 54; 10 знаков — нет: подвид по короткому описанию часто не различить. В «одном товаре» и чате описание-обозначение без ответов на вопросы даёт `low` и проверку «Код — догадка».
