...
# Рассуждение — до кода: когда код шёл первым, модель выбирала, а потом подгоняла довод
# («SAS работает на 12 В, то есть больше 80 В» → 8544429009); на 6 строках пилота × 3 прогона
# код первым — 10 из 18 верно, рассуждение первым — 18 из 18.
TAIL_MAX = 30
...
Сначала рассуждение, потом код: пройди дерево сверху вниз и на каждом уровне скажи, подходит ли товар
под эту строку, по признакам из описания (назначение, материал, напряжение, мощность, размеры); если
признака в описании нет — по тому, каким такой товар бывает обычно. Код — тот, к которому привёл этот путь.

Отвечай СТРОГО в формате JSON:
{"reasoning": "2–4 предложения: уровни дерева сверху вниз, почему этот код, а не соседние", "code": "XXXXXXXXXX"}"""
...
# Группы, где позицию задаёт материал: пластмассы, резина, дерево, пробка, плетёные, бумага,
# камень и бетон, керамика, стекло, чёрные и цветные металлы. Материала в описании нет —
# модель его угадывает по цвету и названию модели («терракотовый», «Кирпич»): на холдинге
# (прогон 20260926-0012) у таких строк позиция верна в 3 из 20, из них 12 были high.
MATERIAL_CHAPTERS = {"39", "40", "44", "45", "46", "48", "68", "69", "70"} | {str(c) for c in range(72, 84)}
MATERIAL_WORDS = re.compile(
    r"пласт|полимер|полипроп|полиэт|полистир|поликарб|пвх|пнд|пвд|акрил|нейлон|полиамид|силикон|резин|каучук|"
    r"металл|стал[ьи]|стальн|нерж|чугун|железн|алюмин|\bмедн|\bмедь|латун|бронз|цинк|оцинк|титан|свинц|\bолов|никел|"
    r"дерев|древес|\bсосн|берез|берёз|\bдуб|фанер|двп|дсп|мдф|бамбук|ротанг|\bлоз|пробк|бумаг|бумаж|картон|"
    r"бетон|цемент|камен|камн|гранит|мрамор|гипс|керам|фарфор|фаянс|\bглин|стекл|кирпич|"
    r"plastic|polyprop|polyeth|pvc|\babs\b|nylon|rubber|silicone|steel|iron|alumin|copper|brass|bronze|zinc|metal|"
    r"wood|bamboo|paper|cardboard|concrete|stone|ceramic|porcelain|glass", re.I)
QUOTED_NAME = re.compile(r"«[^»]*»|\"[^\"]*\"|“[^”]*”|„[^“”]*[“”]")


def _material_unstated(description: str, result: dict) -> None:
    """Код из группы «по материалу», а материала в описании нет → low и проверка.

    Названия в кавычках не считаются: «Кирпич» — модель бордюра, а не материал.
    """
    primary = result["primary"]
    code = primary.get("code") or ""
    if code[:2] not in MATERIAL_CHAPTERS or MATERIAL_WORDS.search(QUOTED_NAME.sub(" ", description)):
        return
    primary["confidence"] = "low"
    primary["confidence_lowered"] = f"материал не назван, а группа {code[:2]} определяется материалом"
    result["checks_required"].insert(0, f"Материал в описании не назван, а позицию {code[:4]} задаёт материал "
                                        f"(группа {code[:2]}) — модель его угадала. Уточнить материал.")
